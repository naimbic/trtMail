  ...
  return (
    <AuthGuard>
      ...
      <MailboxProvider>
        <ComposeProvider>
          <MailSearchProvider>
            <div className="mail-shell grid h-[100dvh] grid-cols-[var(--sidebar-width)_minmax(0,1fr)] overflow-hidden bg-[#f6f8fc] transition-[grid-template-columns] duration-200">
              <aside className="mail-sidebar min-h-0 overflow-y-auto overscroll-contain px-3 py-4 [scrollbar-gutter:stable]">
                <DashboardNav />
              </aside>
              <div className="flex min-h-0 min-w-0 flex-col">
                <header className="mail-topbar flex h-16 w-full shrink-0 items-center gap-4 pr-4 text-sm">
                  <MailSearchInput />
                  <Link
                    href="/settings"
                ...
                <main className="mail-content min-h-0 flex-1 overflow-y-auto overscroll-contain scrollbar-gutter-stable">
                  {children}
                </main>
              </div>
              <FloatingComposer />
